...
| T16 | P1 | RUNNING@aws-hackathon-c2 | qa-verifier | backend/eval_run.py, docs/evidence/eval-* | Build the eval harness for T9: run the scenario set against a base URL and score contract validity, grounding (every sourceUrl in the KB/regulators.json URL set), delay semantics (flag + at-risk + ISO date), unregulated-SWE judgment. Test offline against a stub | passes against a stub server; prints the one-line README accuracy summary |

...
for k, v in {"AWS_REGION": "us-west-2", "AWS_ACCESS_KEY_ID": "offline", "AWS_SECRET_ACCESS_KEY": "offline",
             "AWS_EC2_METADATA_DISABLED": "true"}.items():
...
assert not bad, bad
assert isinstance(a, Agent) and a.model.config["max_tokens"] == 3000 and a.model.config["temperature"] == 0.2
assert h.model.config["max_tokens"] == 3000 and h.model.config["temperature"] == 0.1
...
print("OK  fallback text+parse path")
